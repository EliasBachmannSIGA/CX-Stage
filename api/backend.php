...
switch($method) {
    ...
    case 'POST':
        ...
        if(!$foundAddressKey) {
            ...
            file_put_contents('data.json', json_encode($addresses, JSON_PRETTY_PRINT));
    ...
    case 'DELETE':
        ...
        if ($foundAddressKey == null) {
            http_response_code(400);
            echo json_encode(['message'=> 'Adresse nicht gefunden']);
            exit;
        };

        unset($addresses[$key]); // $foundAddressKey

        file_put_contents('data.json', json_encode($addresses));
        
        header('Content-Type: application/json');
        echo json_encode($foundAddressKey);
        exit;
